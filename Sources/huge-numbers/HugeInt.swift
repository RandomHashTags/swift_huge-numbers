...
extension SIMD {
    func reversed() -> Self {
        let count:Int = scalarCount-1
        var vector:Self = Self.init()
        for i in 0..<scalarCount {
            vector[i] = self[count-i]
        }
        return vector
    }
    func shift_right() -> Self {
        var vector:Self = Self.init()
        var index:Int = 0
        for i in 1..<scalarCount-1 {
            vector[i] = self[index]
            index += 1
        }
        return vector
...
struct HugeInt2 : Hashable {
    var numbers:SIMD64<UInt8>
    
    init(_ string: String) {
        var vector:SIMD64<UInt8> = SIMD64<UInt8>.init(repeating: 0)
        var index:Int = 0
        for i in string.reversed() {
            vector[index] = UInt8(exactly: i.wholeNumberValue!)!
            index += 1
        }
        numbers = vector
    }
    init(numbers: SIMD64<UInt8>) {
        self.numbers = numbers
    }
    
    var description : String {
        let vector:SIMD64<UInt8> = numbers.reversed()
        var index:Int = 0
        while index < 64 && vector[index] == 0 {
            index += 1
        }
        var string:String = ""
        while index < vector.scalarCount {
            string.append(String(describing: vector[index]))
            index += 1
        }
        return string
    }
...
extension HugeInt2 {
    private static var zero:SIMD64<UInt8> = SIMD64<UInt8>(repeating: 0)
    private static var ten:SIMD64<UInt8> = SIMD64<UInt8>(repeating: 10)
    
    static func + (left: HugeInt2, right: HugeInt2) -> HugeInt2 {
        var value:SIMD64<UInt8> = left.numbers &+ right.numbers
        var test:SIMDMask<SIMD64<UInt8.SIMDMaskScalar>> = value .>= ten
        while any(test) {
            let subtracted:SIMD64<UInt8> = zero.replacing(with: ten, where: test)
            let carried:SIMD64<UInt8> = zero.replacing(with: 1, where: test).shift_right()
            //print("carried=" + carried.description)
            value &-= subtracted
            value &+= carried
            
            test = value .>= ten
            //print("subtracted=" + subtracted.description)
        }
        return HugeInt2(numbers: value)
    }
}
