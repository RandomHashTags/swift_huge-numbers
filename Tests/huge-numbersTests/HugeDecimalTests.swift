...
final class HugeDecimalTests : XCTestCase {
    func test_decimal() async {
        ...
        XCTAssert(remainder == expected_remainder, "remainder=\(result);expected_remainder=\(expected_remainder)")
    ...
    func test_decimal_addition() {
        ...
        XCTAssert(result == expected_result && quotient == expected_quotient, "result=\(result);expected_result=\(expected_result);quotient=\(String(describing: quotient));expected_quotient=\(String(describing: expected_quotient))")
        
        (result, quotient) = HugeDecimal("998") + HugeDecimal("001", remove_leading_zeros: false)
        (expected_result, expected_quotient) = (decimal, nil)
        XCTAssert(result == expected_result && quotient == expected_quotient, "result=\(result);expected_result=\(expected_result);quotient=\(String(describing: quotient));expected_quotient=\(String(describing: expected_quotient))")
    }
    func test_decimal_subtraction() {
    ...
    func test_decimal_multiplication() {
